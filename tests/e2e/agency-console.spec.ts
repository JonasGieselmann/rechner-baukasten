...
test('agency invite link onboards a customer into the agency org + scoped pw reset', async ({ browser }) => {
  ...
  // Default invite role is "Als Kunde" — create a customer invite.
  ...
  // Reset-link is the primary action; verify it produces a copyable link.
  await ap.getByRole('button', { name: 'Reset-Link erzeugen' }).click();
  await expect(ap.getByTestId('reset-link-value')).toContainText('/passwort-zuruecksetzen/', { timeout: 10000 });
  // Direct-set still works under the <details>.
  await ap.getByText('Stattdessen direkt ein Passwort setzen').click();
  await ap.getByPlaceholder('Neues Passwort (min. 8 Zeichen)').fill('AgencyReset9!');
  await ap.getByRole('button', { name: 'Passwort direkt setzen' }).click();
