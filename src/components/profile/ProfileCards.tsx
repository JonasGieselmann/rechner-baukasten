...
const ROLE_LABEL: Record<string, string> = {
  super_admin: 'Plattform-Administrator',
...
export function ProfileCard() {
  ...
  const memberSince = createdAt
    ...
    : null;
  const roleLabel = user?.role ? ROLE_LABEL[user.role] ?? null : null;
    ...
    >
      ...
      <div className="space-y-3">
        <div>
          ...
          <input type="text" value={name} onChange={(e) => setName(e.target.value)} className={INPUT_CLS} style={INPUT_STYLE} />
        </div>
        <div>
          <label className="block text-xs font-medium mb-1 opacity-60">E-Mail</label>
          <input type="email" value={user?.email ?? ''} readOnly className={INPUT_CLS + ' opacity-50 cursor-not-allowed'} style={INPUT_STYLE} />
        </div>
      </div>
      {error && <p className="text-sm text-red-500">{error}</p>}
      {success && <p className="text-sm" style={{ color: BRAND.colors.accent }}>Profil gespeichert.</p>}
      <button
        onClick={handleSave}
        disabled={saving}
...
export function PraxisCard() {
  ...
  return (
    <div className="rounded-2xl border p-6 space-y-4" style={{ backgroundColor: BRAND.colors.card, borderColor: BRAND.colors.border }}>
      <h2 className="text-lg font-semibold" style={{ color: BRAND.colors.text }}>Praxis-Angaben</h2>
      <div className="space-y-3">
        <div>
          <label className="block text-xs font-medium mb-1 opacity-60">Telefon</label>
          <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} className={INPUT_CLS} style={INPUT_STYLE} placeholder="+49 123 456789" />
        </div>
        <div>
          <label className="block text-xs font-medium mb-1 opacity-60">Praxisname</label>
          <input type="text" value={businessName} onChange={(e) => setBusinessName(e.target.value)} className={INPUT_CLS} style={INPUT_STYLE} placeholder="Ihr Praxisname" />
        </div>
        <div>
          <label className="block text-xs font-medium mb-1 opacity-60">Website-URL</label>
          <input type="url" value={websiteUrl} onChange={(e) => setWebsiteUrl(e.target.value)} className={INPUT_CLS} style={INPUT_STYLE} placeholder="https://ihre-praxis.de" />
        </div>
        <div>
          <label className="block text-xs font-medium mb-1 opacity-60">Instagram-Handle</label>
          <input type="text" value={instagramHandle} onChange={(e) => setInstagramHandle(e.target.value)} className={INPUT_CLS} style={INPUT_STYLE} placeholder="@ihre_praxis" />
        </div>
        <div>
          <label className="block text-xs font-medium mb-1 opacity-60">Google My Business oder Praxis + Stadt</label>
          <input type="text" value={gmbUrl} onChange={(e) => setGmbUrl(e.target.value)} className={INPUT_CLS} style={INPUT_STYLE} placeholder="z.B. Ihre Praxis Berlin Mitte" />
        </div>
      </div>
      {error && <p className="text-sm text-red-500">{error}</p>}
      {success && <p className="text-sm" style={{ color: BRAND.colors.accent }}>Praxis-Angaben gespeichert.</p>}
      <button
        onClick={handleSave}
        disabled={saving}
        className="inline-flex items-center px-5 py-2 rounded-full text-sm font-semibold transition-opacity hover:opacity-90 disabled:opacity-40"
...
export function PasswordCard() {
  ...
  return (
    <div className="rounded-2xl border p-6 space-y-4" style={{ backgroundColor: BRAND.colors.card, borderColor: BRAND.colors.border }}>
      <h2 className="text-lg font-semibold" style={{ color: BRAND.colors.text }}>Passwort</h2>
      <div className="space-y-3">
        <div>
          <label className="block text-xs font-medium mb-1 opacity-60">Aktuelles Passwort</label>
          <input type="password" value={current} onChange={(e) => setCurrent(e.target.value)} className={INPUT_CLS} style={INPUT_STYLE} autoComplete="current-password" />
        </div>
        <div>
          <label className="block text-xs font-medium mb-1 opacity-60">Neues Passwort</label>
          <input type="password" value={next} onChange={(e) => setNext(e.target.value)} className={INPUT_CLS} style={INPUT_STYLE} autoComplete="new-password" />
        </div>
        <div>
          <label className="block text-xs font-medium mb-1 opacity-60">Neues Passwort bestätigen</label>
          <input type="password" value={confirm} onChange={(e) => setConfirm(e.target.value)} className={INPUT_CLS} style={INPUT_STYLE} autoComplete="new-password" />
        </div>
      </div>
      {error && <p className="text-sm text-red-500">{error}</p>}
      {success && <p className="text-sm" style={{ color: BRAND.colors.accent }}>Passwort erfolgreich geändert.</p>}
      <button
...
export function RechtlichesCard() {
  return (
    <div className="rounded-2xl border p-6 space-y-3" style={{ backgroundColor: BRAND.colors.card, borderColor: BRAND.colors.border }}>
      <h2 className="text-lg font-semibold" style={{ color: BRAND.colors.text }}>Rechtliches &amp; Datenschutz</h2>
...
export function DeleteAccountCard() {
  ...
  return (
    <div className="rounded-2xl border p-6 space-y-3" style={{ backgroundColor: BRAND.colors.card, borderColor: BRAND.colors.border }}>
      <h2 className="text-lg font-semibold" style={{ color: BRAND.colors.text }}>Konto</h2>
      ...
      {!confirming ? (
        ...
        <div className="flex flex-wrap items-center gap-3">
          <span className="text-sm font-medium" style={{ color: BRAND.colors.text }}>Wirklich endgültig löschen?</span>
          <button
          ...
          <button onClick={() => setConfirming(false)} disabled={deleting} className="text-sm transition-opacity hover:opacity-70" style={{ color: BRAND.colors.muted }}>
            Abbrechen
          </button>
        </div>
      )}
